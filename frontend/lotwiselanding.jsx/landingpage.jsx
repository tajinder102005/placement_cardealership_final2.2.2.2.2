...
function Modal({ mode, onClose, onSwitch }) {
    ...
    return (
        ...
        >
            ...
            >
                <button className="lw-modal-close" onClick={onClose}><X size={18} /></button>
                <div className="lw-modal-mark">
                    <Car size={18} strokeWidth={2.5} />
                </div>
                <h3 className="lw-modal-title">{isLogin ? "Welcome back" : "Set up your lot"}</h3>
                <p className="lw-modal-sub">
                    {isLogin ? "Log in to pick up right where your team left off." : "Create your account — no card required to start."}
                </p>

                <div className="lw-field">
                    <User size={15} />
                    <input placeholder={isLogin ? "Email or dealer ID" : "Full name"} />
                </div>
                {!isLogin && (
                    <div className="lw-field">
                        <Mail size={15} />
                ...
                <button className="lw-btn-primary lw-modal-submit">
                    {isLogin ? "Log in" : "Create account"} <ChevronRight size={16} />
                </button>

                <div className="lw-modal-switch">
                    {isLogin ? "New to Lotwise?" : "Already have an account?"}{" "}
    ...
    return (
        <div className="lw-root">
            ...
            <header className={`lw-nav ${scrolled ? "lw-nav-scrolled" : ""}`}>
                <div className="lw-nav-inner">
                    ...
                    <div className="lw-nav-actions">
                        <button className="lw-btn-ghost lw-btn-small" onClick={() => setModal("login")}>Log in</button>
                        <button className="lw-btn-primary lw-btn-small" onClick={() => setModal("signup")}>Sign up</button>
                    </div>
                </div>
            </header>

            <Hero onGetStarted={setModal} />
            ...
            <section className="lw-cta-footer">
                ...
                <button className="lw-btn-primary lw-btn-large" onClick={() => setModal("signup")}>
            ...
            <AnimatePresence>
                {modal && <Modal mode={modal} onClose={() => setModal(null)} onSwitch={setModal} />}
            </AnimatePresence>
        </div>
    );
}
